...
function CaptureMouse(t) {
    ...
    return t.addEventListener("mousemove", function (e) {
        var o = WindowToCanvas(t, e.clientX, e.clientY);
        n.x = o.x, n.y = o.y
    }, !1), n
}
...
function IsCollision(t, n, e, o) {
    var r = (t.y - n.y) / (t.x - n.x), a = (o.y - e.y) / (o.x - n.x), i = t.y - r * t.x, u = e.y - a * e.x,
...
var Color = {
    rgbToHex: function (t) {
        ...
        if (/^rgba?|RGBA?/.test(t)) {
            ...
            for (var r = 0; r < 3; r++) o = Math.min(parseInt(e[r], 10), 255).toString(16), n += ("0" + o).slice(-2);
            n = "#" + n
        }
        return n
    }, hexToRgb: function (t) {
        var n = [], e = t.toLowerCase();
        if (/^#([0-9a-f]{3}|[0-9a-f]{6})$/.test(e)) for (var o = 4 == e.length ? 1 : 2, r = 1; r < 3 * o + 1; r += o) n.push(parseInt("0x" + new Array(4 - o).join(e.slice(r, r + o))));
        return n
    }, rgbToHsl: function (t) {
        var n, e, o, r = t[0] / 255, a = t[1] / 255, i = t[2] / 255, u = Math.max(r, a, i), s = Math.min(r, a, i);
        o = (u + s) / 2;
        var c = u - s;
        ...
        var n = parseFloat(t[0] / 360, 10), e = parseFloat(t[1] / 100, 10), o = parseFloat(t[2] / 100, 10);
        if (0 == e) {
        ...
        if (/^hsla?|HSLA?/.test(t)) {
            var n = t.match(/\d+/g);
            if (n && n.length >= 3) {
                var e = "rgb(" + this.hslToRgb(n).join(",") + ")";
    ...
    getBezierAngle: function (t, n, e, o, r) {
        var a = 3 * t.x * Math.pow(1 - r, 2) * -1 + 3 * n.x * (Math.pow(1 - r, 2) + 2 * r * (1 - r) * -1) + 3 * e.x * (2 * r * (1 - r) + r * r * -1) + 3 * o.x * r * r,
        ...
        var r = 2 * t.x * (1 - o) * -1 + 2 * n.x * (1 - o + -1 * o) + 2 * e.x * o,
